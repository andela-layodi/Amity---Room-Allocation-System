...
class Amity(object):
    ...
    def save_state(self, db_name):
        ...
        data = (self.rooms, self.persons)
        data_dump = pickle.dumps(data)
        data_bin = sqlite3.Binary(data_dump)
        with conn:
            cur = conn.cursor()
            cur.execute("DROP TABLE IF EXISTS Parameters")
            cur.execute("CREATE TABLE Parameters(Data BLOB)")
            # for k, v in self.rooms.items():
            #     cols = v.keys()
            #     vals = v.values()
            cur.execute("INSERT INTO Parameters(Data) VALUES (?)", (data_bin,))
                # try:
    ...
    def load_state(self, db_name):
        ...
        with conn:
            cur = conn.cursor()
            cur.execute("SELECT Data FROM Parameters")
            data_bin = cur.fetchone()[0]
            data = pickle.loads(data_bin)
            self.rooms, self.persons = data
